...
label funq1:
    if current_ans==100:
        ...
        h "Eso no es correcto"
    $ cur_var = preguntas[current_ans]
    
    
    menu:
        "#Dado el siguiente código:\nx=3\ndef funcion():\n\ \ \ \ y=2\n\ \ \ \ z=\"UnElemento\"\n¿Cuál es el ambito de la variable [cur_var]?"
        "local":
            $ resp="local"
            jump funq1
        "global":
...
label funciones2:
    ...
    h "Ahora mira este fragmento del código de este tutorial ubicado en la linea 570 del archivo screens.rpy"
    h "570 textbutton _(\"Cambiar la musica\") action Function(cambiarMusica, musica)"
    ...
    h "{color=#ff0}def cambiarMusica(m):{/color}\nEn la sentencia donde se define la función: {color=#ff0}def cambiarMusica(m):{/color} vemos que hay un argumento que llamé m, que veremos que hace más adelante"
    h "{color=#ff0}global musica, now_playing{/color}Luego aparece la en la siguiente sentencia palabra reservada {color=#ff0}global{/color}, la cual permite a la función acceder a las variables {color=#ff0}musica{/color} y {color=#ff0}now_playing{/color} del ámbito global\nes decir que esta palabra reservada, permite acceder a variables fuera del ambito local de la función, al igual que modificarlas"
    h "{color=#ff0}lm=\['The Show Must Be Go.mp3','Poofy Reel.mp3']{/color}\n, creo una lista como variable interna lm que contiene las melodías mp3 que se pueden tocar" 
    h "En la siguiente línea: lm.remove(m)\n, mediante el método remove (que es una función de los objetos lista) elimino cualquier objeto de la lista que sea igual a la variable local y argumento m"
...
label class4_q:
    ...
    menu:
        ...
        "[menu3]":
            ...
            jump class4_q
label class5:
    h "Hemos casi finalizado esta historia"
    h "Para finalizar voy a hablar de {color=#ff0}pip{/color}"
    h "{color=#ff0}pip{/color} es un instalar de módulos de python los cuales no están incluidos en la instalación"
    h "El módulo math, no require pip, pero módulos como numpy (módulo para funciones matemáticas avanzadas) o kivy (módulo para realizar apps) requiren instalarse aparte"
    h "En lo personal prefiero usar el instalador, pero pip puede ser una buena opción"
    h "Para instalarlo, hay que bajar el script get-pip.py de la {a=https://pip.pypa.io/en/stable/installing/}página web oficial{/color}"
    h "Y luego correr rl programa desde la línea de comandos o usando el IDLE de python"
    h "{color=#ff0}pip{/color} permite instalar módulos necesarios para el desarrollo de un proyecto"
    h "Creo que tienes ya todas las herramientas mínimas para usar python... ¡pero ahora te toca a tí!"
    h "Te toca leer la documentación especifica para cada proyecto..."
    h "Si quieres me puedes contactar, lo puedes hacer en mis redes sociales, via twitter en @hedleypanama y te contestaré cuando pueda"
    h "Si crees que lo merezco puedes donar a mi cuenta de paypal"
    h "Como dicen los gringos {color=00f}happy coding{\color}"
    return
